...
- **클라우드 저장**: Supabase를 통한 안전한 데이터 저장 및 동기화
...
### 4. 사용자 인증
- **이메일 로그인**: Supabase Auth를 통한 안전한 인증
- **세션 관리**: 자동 로그인 상태 유지
- **로그아웃**: 안전한 세션 종료
...
1. **Supabase 설정**
   ...
   - 프로젝트 URL과 anon key를 `src/integrations/supabase/client.ts`에 설정

2. **Gemini API 설정**
   - [Google AI Studio](https://aistudio.google.com/)에서 Gemini 2.0 Flash API 키 발급
   - 앱에서 "Gemini API 설정" 섹션에서 API 키 입력 및 저장

## 📱 사용법

### 인증
1. 앱 접속 시 로그인 페이지로 이동
2. 이메일 주소 입력하여 로그인
3. 이메일로 받은 링크 클릭하여 인증 완료

### 일기 작성
1. "일기 작성" 탭에서 날짜 선택
2. 오늘의 감정 선택
3. 3가지 감사 항목 작성 (각 항목에 여러 개 입력 가능)
4. "저장하기" 버튼 클릭
5. AI가 생성한 문학적 한줄 일기 확인

### 일기 관리
- **일기 목록**: "일기 목록" 탭에서 모든 일기 확인 (감정과 한줄 요약 표시)
...
- 사용자 인증은 Supabase Auth를 통해 처리됩니다
...
- [x] 감사일기 작성 (동적 항목 지원)
- [x] AI 한줄 일기 요약 (Gemini 2.0 Flash)
- [x] 일기 저장 및 목록 (Supabase 데이터베이스 연동)
- [x] 월간 감정 리포트
- [x] 긍정률 계산 (행복, 기쁨 기준)
- [x] 일기 수정 및 삭제
- [x] 반응형 UI (모바일/데스크톱)
